...
bool gcd_complete;          // Signal to check if the gcd operation has completed.
bool lcm_complete;          // Signal to check if the lcm operation has completed.
...
int get_lcm_inst(int pc, int b) {
    if (pc == start_position) return 0x00800193; // r3 = r0 + 8
    if (pc == start_position + 4) return 0x00900213; // r4 = r0 + 9
    if (pc == start_position + 8) return 0x023201B3; // r3 = r3 * r4
    if (pc == start_position + 12) return 0x0221C133 // r2 = r3 / r2;
...
int main(int argc, char **argv) {
    ...
    // tick the clock until we are done
        ...
        while (gcd_complete != true) {
        tb->riscv->mem_rdata_0 = get_gcd_inst(position, tb->riscv->r2);
        next_time_frame(tb, time, tfp);
        ...
        while (lcm_complete != true) {
        tb->riscv->mem_rdata_0 = get_lcm_inst(position);
        next_time_frame(tb, time, tfp);
        position = tb->riscv->pc;
        }
