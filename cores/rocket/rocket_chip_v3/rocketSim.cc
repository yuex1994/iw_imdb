...
void print_out(VRocket* cpu) {
    printf("io_imem_resp_ready: %ud \n", cpu->io_imem_resp_ready);
    printf("io_imem_req_valid: %ud \n", cpu->io_imem_req_valid);
    printf("io_imem_req_bits_pc: %ud \n", cpu->io_imem_req_bits_pc);
...
int main(int argc, char **argv) {
	...
	*/
    ...
    tb->io_imem_resp_bits_data = 0x00100113;
    ...
    for (int i = 0; i < 50; i++) {
        next_time_frame(tb, time, tfp);
        tb->io_imem_resp_bits_pc = tb->io_imem_req_bits_pc;
        print_out(tb);
    }
