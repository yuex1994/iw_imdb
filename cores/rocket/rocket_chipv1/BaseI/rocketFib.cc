...
int main(int argc, char **argv) {
    ...
    // tick the clock until we are done
        ...
        // Fiboonacci usig space optimized method in CPU module registers.

        tb->riscv->x1 = 0; /* first fibonacci number */
        tb->riscv->x2 = 1;  /* second fibonacci number */

        int fibonacci = 6;

        for (int i = 0; i < 100; i++) {
            next_time_frame(tb, time, tfp); /* tick the clock */
        }

        printf("%d<-value in r1 after clock ticking loop", tb->riscv->x1);
        printf("%d<-value in r2 after clock ticking loop", tb->riscv->x2);
        printf("%d<-value in r3 after clock ticking loop", tb->riscv->x3); /* check register starting values */

        for (int i = 2; i < fibonacci; i++) {
            tb->riscv->mem_rdata_0 = 0x000100B3; /* r1 = r2 */
            next_time_frame(tb, time, tfp);
            next_time_frame(tb, time, tfp);
            tb->riscv->mem_rdata_0 = 0x00018133; /* r2 = r3 */
            next_time_frame(tb, time, tfp);
            next_time_frame(tb, time, tfp);
            tb->riscv->mem_rdata_0 = 0x002081B3; /* r3 = r1 + r2 */
            next_time_frame(tb, time, tfp);
            next_time_frame(tb, time, tfp);        
            }

        // printf("%x instruction at exit", tb->riscv->mem_rdata_0);
