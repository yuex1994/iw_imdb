...
bool check_state_valid(VmkCPU* cpu, riscv ila_) {
    ...
    mem_valid = (ila_.riscv_ren == cpu->monitor_s2 && cpu->mkCPU->near_mem->dmem_valid) || (ila_.riscv_raddr == cpu->mkCPU->near_mem->dmem_req_addr) ||
                (ila_.riscv_rdata == cpu->mkCPU->near_mem->dmem_word64) || (ila_.riscv_wen == (cpu->monitor_s2 && cpu->mkCPU->near_mem->dmem_req_op != 0)) ||
                (ila_.riscv_waddr == cpu->mkCPU->near_mem->dmem_req_addr) || (ila_.riscv_wdata == /* write data wire in nearmem module in RTL */)
                // compare all of the in/out ports for memory on each execution of the ILA model
                // if all signals are the same on each clock cycles, assuming the designs are
                // correct and working, then contents of memory should be the same.
...
int main(int argc, char **argv) {
  ...
  print_out(tb);
    ...
    while (!testVector.empty() || !fetchedQueue.empty()) {
        ...
        if (// RTL_end_of_inst) {                   // RTL_end_of_inst = rg_retiring$EN?
    ...
    // The form of testVector is all of the instructions to be executed, including repeat looped instructions. I.e, if
    // we had 10 additions to make, there would be 10 probably identical add instructions in the vector?

    // Found I could not really push the instructions elegantly, since a branch, for example, would require that I call a
    // next_time_frame so the pc changes value and I can use the Instruction Memory map to load another instruction into the test vector.

    // Now I am just going to call get_instruction from the main() and then push those instructions on the fetchedQueue for the ILA.

    // Also it seems this format needs us to expand all instructions beforehand and we cannot compactly
    // test instructions with complex logical flow, like branches....my guess is the FIFO does not repush
    // instructions to be executed in a loop. So adding several times could not be achieved by a branch-loop.

    // There are two different cases of the tandem simulation loop, one being a stored program execution
    // in which instructions are retrieved from a memory map, and the other being a serial execution
    // of instructions that are fed in as one sequence, this would not allow for complex logical
    // branching with the current code on file.
