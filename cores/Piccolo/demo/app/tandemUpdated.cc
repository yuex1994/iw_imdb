...
int main(int argc, char **argv) {
  ...
  print_out(tb);
    ...
    ila_.riscv_pc = position;	// set the pc's to be equal
    ...
    for (int i = 0; i < 4; i++) {
	ila_.riscv_mem[position] = get_instruction(position);
	position+=4;
    }
    ...
    while (!testVector.empty() || !fetchedQueue.empty()) {
        ...
        if (tb->mkCPU->rg_retiring$EN) {             
            ila_.compute();   /* when an RTL instruction completes, compute the next ready instruction in ILA, both of which should correspond            
	   if (check_status(tb, ila_) printf("Simulation error occurred");
        }
	next_time_frame(tb, time, tfp);
